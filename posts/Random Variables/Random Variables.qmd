...
A random variable is simply a mapping from a sample space to the real line. Imagine performing an experiment, say throwing a die twice, you might be interested in a numerical value such as the following :

a)  Number of six observed

b)  The sum of the outcomes of the two rolls

c)  even numbers observed

This are all examples of random variables of our experiment. One thing that we can observe is that from one experiment we can have different random variable and each one is also not surprisinfly random because its values are derived from the sample space which is itself random.
